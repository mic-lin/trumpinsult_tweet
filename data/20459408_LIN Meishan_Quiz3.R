...
# 1.1 import data ----- 
covid19_tweet_01_3 <- read.csv(file.choose(), header = TRUE, encoding="UTF-8", stringsAsFactors=FALSE)  # file.choose()
# covid19_tweet_01_3_Moodle_3Mar2020_v3.csv
dim(covid19_tweet_01_3)
names(covid19_tweet_01_3)
head(covid19_tweet_01_3) 

# 1.2 Inspecting the dataset and variables (with EDAV)  ----- 

names(covid19_tweet_01_3)
names(covid19_tweet_01_3)[names(covid19_tweet_01_3) == 'X'] <- 'doc_id'

class(covid19_tweet_01_3$screen_name) # character 
unique(covid19_tweet_01_3$screen_name) 

class(covid19_tweet_01_3$profile)
head(covid19_tweet_01_3$profile, 2)

class(covid19_tweet_01_3$followers_count.x) 
names(covid19_tweet_01_3)[names(covid19_tweet_01_3) == 'followers_count.x'] <- 'followers_count'
covid19_tweet_01_3$followers_count <- as.numeric(covid19_tweet_01_3$followers_count)
describe(covid19_tweet_01_3$followers_count) 

class(covid19_tweet_01_3$friends_count.x) 
names(covid19_tweet_01_3)[names(covid19_tweet_01_3) == 'friends_count.x'] <- 'friends_count'
covid19_tweet_01_3$friends_count <- as.numeric(covid19_tweet_01_3$friends_count)
describe(covid19_tweet_01_3$friends_count)

class(covid19_tweet_01_3$statuses_count.x) 
names(covid19_tweet_01_3)[names(covid19_tweet_01_3) == 'statuses_count.x'] <- 'statuses_count'
covid19_tweet_01_3$statuses_count <- as.numeric(covid19_tweet_01_3$statuses_count)
describe(covid19_tweet_01_3$statuses_count)

class(covid19_tweet_01_3$verified.x) # what type is it? 
covid19_tweet_01_3$verified_1 <- as.factor(covid19_tweet_01_3$verified.x)
class(covid19_tweet_01_3$created_at) # please note that R has special way to handle the time format

class(covid19_tweet_01_3$gender) # integar? 
covid19_tweet_01_3$gender <- as.factor(covid19_tweet_01_3$gender)
table(covid19_tweet_01_3$gender)

class(covid19_tweet_01_3$media_name) # integar?
covid19_tweet_01_3$media_name <- as.factor(covid19_tweet_01_3$media_name)
class(covid19_tweet_01_3$media_type) # integar?
covid19_tweet_01_3$media_type <- as.factor(covid19_tweet_01_3$media_type)
class(covid19_tweet_01_3$text) # character 
names(covid19_tweet_01_3)[names(covid19_tweet_01_3) == 'text_x'] <- 'text'

class(covid19_tweet_01_3$retweet_count) # 
covid19_tweet_01_3$retweet_count <- as.numeric(covid19_tweet_01_3$retweet_count)
describe(covid19_tweet_01_3$retweet_count)
hist(covid19_tweet_01_3$retweet_count)
covid19_tweet_01_3$retweet_count_log <- log(covid19_tweet_01_3$retweet_count + 1)  # log by default is the *natural logarithm* of the value
describe(covid19_tweet_01_3$retweet_count_log)
hist(covid19_tweet_01_3$retweet_count_log)

# 1.3 unsupervised machine learning for text analytics -----
'
Media name	
0 - Huffington Post**
1 - BuzzFeed**
...
# preparing the dataset for online media (Huffinton Post and BuzzFeed News) and traditional media (others)
covid19_tweet_online <- covid19_tweet_01_3[which(covid19_tweet_01_3$media_name==0 | covid19_tweet_01_3$media_name==1), ]
covid19_tweet_traditional <- covid19_tweet_01_3[which(covid19_tweet_01_3$media_name==2|covid19_tweet_01_3$media_name==3| covid19_tweet_01_3$media_name==4| covid19_tweet_01_3$media_name==5| covid19_tweet_01_3$media_name==6), ]
table(covid19_tweet_online$media_name)
table(covid19_tweet_traditional$media_name)

library(NLP)
library(tm)

# 1.3.1 Data cleaning for text data  -----

# (analyzing the online media ) 
tweets_online_docs <- subset(covid19_tweet_online, select = c("doc_id", "text"))
tweets_online_VCorpus <- VCorpus(DataframeSource(tweets_online_docs)) 

# to inspect the contents 
tweets_online_VCorpus[[1]]$content

# to convert to all lower cases
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, content_transformer(tolower))  # tm_map: to apply transformation functions (also denoted as mappings) to corpora
tweets_online_VCorpus[[1]]$content

# to remove URLs 
removeURL <- function(x) gsub("http[^[:space:]]*", "", x)
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, content_transformer(removeURL)) 

# to remove anything other than English 
removeNumPunct <- function(x) gsub("[^[:alpha:][:space:]]*", "", x)
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, content_transformer(removeNumPunct)) 

# to remove stopwords, (note: one can define their own myStopwords) 
stopwords("english")
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, removeWords, stopwords("english"))

# to remove extra whitespaces 
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, stripWhitespace) 

# to remove punctuations 
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, removePunctuation)
tweets_online_VCorpus[[1]]$content
...
library(SnowballC)
tweets_online_VCorpus <- tm_map(tweets_online_VCorpus, stemDocument)
tweets_online_VCorpus[[1]]$content
...
tweets_online_dtm <- DocumentTermMatrix(tweets_online_VCorpus, control = list(removePunctuation = TRUE, stopwords=TRUE)) 
tweets_online_dtm
# A high sparsity means terms are not repeated often among different documents.
inspect(tweets_online_dtm) # a sample of the matrix 

# TF
term_freq_online <- colSums(as.matrix(tweets_online_dtm)) 
write.csv(as.data.frame(sort(rowSums(as.matrix(term_freq_online)), decreasing=TRUE)), file="tweets_online_dtm_tf.csv")

# TF-IDF
tweets_online_dtm_tfidf <- DocumentTermMatrix(tweets_online_VCorpus, control = list(weighting = weightTfIdf)) # DTM is for TF-IDF calculation 
print(tweets_online_dtm_tfidf) 
tweets_online_dtm_tfidf2 = removeSparseTerms(tweets_online_dtm_tfidf, 0.99)
print(tweets_online_dtm_tfidf2) 
write.csv(as.data.frame(sort(colSums(as.matrix(tweets_online_dtm_tfidf2)), decreasing=TRUE)), file="tweets_online_dtm_tfidf.csv")


# 1.3.3 topic modeling with LDA-----
...
rowTotals_online <- apply(tweets_online_dtm , 1, sum) #Find the sum of words in each Document
tweets_online_dtm_nonzero <- tweets_online_dtm[rowTotals_online> 0, ]

# A side note: determining the number of topics
...
## k4 - 4 topics, 10 terms
tweets_online_dtm_4topics <- LDA(tweets_online_dtm_nonzero, k = 4, method = "Gibbs", control = list(iter=2000, seed = 2000)) # find k=6 topics, after the ldatuning
tweets_online_dtm_4topics_10words <- terms(tweets_online_dtm_4topics, 10) # get top 10 words of every topic
tweets_online_dtm_4topics_10words


# 1.4 replicating steps for the traditional media -----

# (analyzing the traditional media ) 
tweets_traditional_docs <- subset(covid19_tweet_traditional, select = c("doc_id", "text"))
tweets_traditional_VCorpus <- VCorpus(DataframeSource(tweets_traditional_docs)) 

# to inspect the contents 
tweets_traditional_VCorpus[[1]]$content

# to convert to all lower cases
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, content_transformer(tolower))  # tm_map: to apply transformation functions (also denoted as mappings) to corpora
tweets_traditional_VCorpus[[1]]$content

# to remove URLs 
removeURL <- function(x) gsub("http[^[:space:]]*", "", x)
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, content_transformer(removeURL)) 

# to remove anything other than English 
removeNumPunct <- function(x) gsub("[^[:alpha:][:space:]]*", "", x)
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, content_transformer(removeNumPunct)) 

# to remove stopwords, (note: one can define their own myStopwords) 
stopwords("english")
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, removeWords, stopwords("english"))

# to remove extra whitespaces 
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, stripWhitespace) 

# to remove punctuations 
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, removePunctuation)
tweets_traditional_VCorpus[[1]]$content

# Stemming: to remove plurals and action suffixes (please use it with caution: some hypertextual elements such as @mentions, #hashtags, and URLs are removed)
library(SnowballC)
tweets_traditional_VCorpus <- tm_map(tweets_traditional_VCorpus, stemDocument)
tweets_traditional_VCorpus[[1]]$content

# converting to Document-term matrix (TDM)
tweets_traditional_dtm <- DocumentTermMatrix(tweets_traditional_VCorpus, control = list(removePunctuation = TRUE, stopwords=TRUE)) 
tweets_traditional_dtm
# A high sparsity means terms are not repeated often among different documents.
inspect(tweets_traditional_dtm) # a sample of the matrix 

# TF
term_freq_traditional <- colSums(as.matrix(tweets_traditional_dtm)) 
write.csv(as.data.frame(sort(rowSums(as.matrix(term_freq_traditional)), decreasing=TRUE)), file="tweets_traditional_dtm_tf.csv")

# TF-IDF
tweets_traditional_dtm_tfidf <- DocumentTermMatrix(tweets_traditional_VCorpus, control = list(weighting = weightTfIdf)) # DTM is for TF-IDF calculation 
print(tweets_traditional_dtm_tfidf) 
tweets_traditional_dtm_tfidf2 = removeSparseTerms(tweets_traditional_dtm_tfidf, 0.99)
print(tweets_traditional_dtm_tfidf2) 
write.csv(as.data.frame(sort(colSums(as.matrix(tweets_traditional_dtm_tfidf2)), decreasing=TRUE)), file="tweets_traditional_dtm_tfidf.csv")
...
# clean the empty (non-zero entry) 
rowTotals_traditional <- apply(tweets_traditional_dtm , 1, sum) #Find the sum of words in each Document
tweets_traditional_dtm_nonzero <- tweets_traditional_dtm[rowTotals_traditional> 0, ]
...
result <- FindTopicsNumber(
  tweets_traditional_dtm_nonzero,
  topics = seq(from = 2, to = 15, by = 1),
  metrics = c("CaoJuan2009", "Arun2010", "Deveaud2014",'Griffiths2004'),
  method = "Gibbs",
  control = list(seed = 77),
...
# after finding "the optimal-K" topics, then redo the above analysis
tweets_traditional_dtm_4topics <- LDA(tweets_traditional_dtm_nonzero, k = 4, method = "Gibbs", control = list(iter=2000, seed = 2000)) 
tweets_traditional_dtm_4topics_10words <- terms(tweets_traditional_dtm_4topics, 10) # get top 10 words of every topic
tweets_traditional_dtm_4topics_10words
